...
|------|-------------|-------|
| 1.1 | Create pricing.py module | `target/shared/scripts/scaling/pricing.py` |
| 1.2 | Create cost_tracker.py module | `target/shared/scripts/scaling/cost_tracker.py` |
| 1.3 | Update Rust FMI communicator for S3/Redis | `rust/src/net/fmi/communicator.rs` |
| 1.4 | Update C++ FMIConfig | `cpp/src/cylon/net/fmi/fmi_config.hpp` |
| 1.5 | Update pycylon Cython bindings | `python/pycylon/pycylon/net/fmi_config.pyx` |

### Phase 2: Experiment Updates (Week 2)

| Task | Description | Files |
|------|-------------|-------|
| 2.1 | Integrate cost tracking in scaling.py | `target/shared/scripts/scaling/scaling.py` |
| 2.2 | Add timing breakdown instrumentation | `target/shared/scripts/scaling/scaling.py` |
| 2.3 | Add S3/Redis channel env options | `target/shared/scripts/scaling/scaling.py` |
| 2.4 | Update Step Function JSON | `target/aws/scripts/step functions/ServerlessCylonExecutor.json` |
| 2.5 | Add GroupBy operation support | `target/shared/scripts/scaling/scaling.py` |

### Phase 3: Run Experiments (Week 3)

| Task | Description | Infrastructure |
|------|-------------|----------------|
| 3.1 | Re-run Join with cost tracking | Lambda, EC2, Rivanna |
| 3.2 | Run S3 baseline experiments | Lambda only |
| 3.3 | Run GroupBy experiments | Lambda, EC2, Rivanna |
| 3.4 | Run communication microbenchmarks | Lambda, EC2, Rivanna |

### Phase 4: Paper Updates (Week 4)

| Task | Description | Section |
|------|-------------|---------|
| 4.1 | Add Contributions subsection | Section I |
| 4.2 | Add cost analysis results | Section IV |
| 4.3 | Add S3 baseline comparison | Section IV |
| 4.4 | Add GroupBy results | Section IV |
| 4.5 | Clarify <1% metric | Section IV |
| 4.6 | Add compute/comm breakdown | Section IV |
| 4.7 | Update figures and tables | Throughout |
...
│   │   └── scripts/
│   │       └── scaling/
│   │           ├── scaling.py          # Main experiment script
│   │           ├── pricing.py          # NEW: Pricing config module
│   │           ├── cost_tracker.py     # NEW: Cost tracking module
│   │           └── aws_pricing.json    # NEW: Default pricing config
│   └── aws/
│       └── scripts/
│           └── step functions/
│               └── ServerlessCylonExecutor.json  # Updated with cost params
├── rust/
│   └── src/
│       └── net/
│           └── fmi/
│               ├── communicator.rs     # Updated for S3/Redis selection
│               ├── s3_channel.rs       # Existing
│               └── redis_channel.rs    # Existing
├── cpp/
│   └── src/
│       └── cylon/
│           └── net/
│               └── fmi/
│                   └── fmi_config.hpp  # Updated with channel_type
├── python/
│   └── pycylon/
│       └── pycylon/
│           └── net/
│               └── fmi_config.pyx      # Updated Cython bindings
└── papers/
    └── frontiers_revision_plan.md      # This document
```
