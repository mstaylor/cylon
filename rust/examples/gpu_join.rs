...
//! GPU Distributed Join Example
//!
//! Demonstrates distributed join operation using gcylon.
//! Similar to the C++ gjoin example.
//!
//! Run with: mpirun -n 2 cargo run --example gpu_join --features gpu

use std::time::Instant;
use cylon::gpu::{GpuContext, GpuConfig, JoinType, get_device_count, set_device};
...
fn main() -> CylonResult<()> {
    ...
    println!("Rank {}: Creating tables with {} columns, {} rows each", rank, cols, rows);

    // Create left and right tables for join
    // Use different start values to create distinct but overlapping data
    let left_start = rank as i64 * rows;
    let right_start = rank as i64 * rows + (rows / 2);

    let left_table = ctx.create_sequential_table(cols, rows, left_start, 1)?;
    let right_table = ctx.create_sequential_table(cols, rows, right_start, 1)?;

    println!(
        "Rank {}: Left table: {} rows, Right table: {} rows",
        rank,
        left_table.num_rows(),
        right_table.num_rows()
    );

    // Perform distributed inner join on column 0
    let config = GpuConfig::default();
    let start = Instant::now();
    let joined = left_table.distributed_join(
        &right_table,
        &[0],     // left join columns
        &[0],     // right join columns
        JoinType::Inner,
        Some(config),
    )?;
    ...
    println!(
        "Rank {}: Distributed join (INNER) completed in {:?}, output has {} rows, {} columns",
        rank,
        duration,
        joined.num_rows(),
        joined.num_columns()
    ...
    if rank == 0 {
        println!("\n========================================");
        println!("GPU DISTRIBUTED JOIN COMPLETED SUCCESSFULLY");
        println!("========================================\n");
    }
