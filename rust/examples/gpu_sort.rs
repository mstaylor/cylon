...
//! GPU Distributed Sort Example
//!
//! Demonstrates distributed sort operation using gcylon.
//! Similar to the C++ gsort example.
//!
//! Run with: mpirun -n 2 cargo run --example gpu_sort --features gpu
...
fn main() -> CylonResult<()> {
    ...
    println!("Rank {}: Creating random table with {} columns, {} rows", rank, cols, rows);

    // Create random data table for sorting
    let seed = 42 + rank; // Different seed per rank
    let table = ctx.create_random_table(cols, rows, seed)?;
    println!(
    ...
    // Perform distributed sort on columns 0 and 1, both ascending
    let sort_columns = [0, 1];
    let ascending = [true, true];
    let config = GpuConfig::default();

    let start = Instant::now();
    let sorted = table.distributed_sort(&sort_columns, &ascending, Some(config))?;
    let duration = start.elapsed();

    println!(
        "Rank {}: Distributed sort completed in {:?}, output has {} rows",
        rank,
        duration,
        sorted.num_rows()
    );
    ...
    if rank == 0 {
        println!("\n========================================");
        println!("GPU DISTRIBUTED SORT COMPLETED SUCCESSFULLY");
        println!("========================================\n");
    }
