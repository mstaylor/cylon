...
//! Distributed Union Example
//!
//! This example demonstrates distributed union operations using MPI.
//! It reads two tables and performs a distributed union operation.
//!
//! Corresponds to cpp/src/examples/union_example.cpp
//!
//! Usage:
//!   mpirun -n 4 cargo run --example union_example --features mpi -- m 1000 0.5
//!   mpirun -n 4 cargo run --example union_example --features mpi -- f file1 file2
...
fn create_two_in_memory_tables(
    ...
    // Generate right table data (offset to create some overlap)
    let mut right_col1: Vec<i64> = Vec::with_capacity(count as usize);
    let mut right_col2: Vec<i64> = Vec::with_capacity(count as usize);

    for i in 0..unique_count {
        // Offset by half to create partial overlap
        let val = (i + unique_count / 2) * world_size + rank as i64;
        right_col1.push(val);
        right_col2.push(val * 10);
    }

    for i in 0..dup_count {
        let val = ((i % unique_count.max(1)) + unique_count / 2) * world_size + rank as i64;
        right_col1.push(val);
        right_col2.push(val * 10);
    }
...
fn print_usage() {
    eprintln!("Usage:");
    eprintln!("  ./union_example m num_tuples_per_worker 0.0-1.0");
    eprintln!("  ./union_example f csv_file1 csv_file2");
}
...
fn main() -> CylonResult<()> {
    ...
    println!(
        "Rank {}: First table had: {} and Second table had: {}, Union has: {} rows",
        rank,
        first_table.rows(),
        second_table.rows(),
        unioned_table.rows()
    ...
    if rank == 0 {
        ...
        println!("Union example completed successfully!");
    }
