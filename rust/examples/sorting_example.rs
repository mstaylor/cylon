...
//!
//! This example demonstrates distributed sorting operations using MPI.
//! It reads a table and performs a distributed sort operation.
//!
//! Corresponds to cpp/src/examples/sorting_example.cpp
//!
//! Usage:
//!   mpirun -n 4 cargo run --example sorting_example --features mpi -- m 1000 0.5
//!   mpirun -n 4 cargo run --example sorting_example --features mpi -- f csv_file
...
fn create_in_memory_table(
    ...
    // Generate unique values (distributed across ranks) in reverse order for sorting demo
    for i in 0..unique_count {
        // Reverse order to demonstrate sorting
        let val = ((unique_count - i - 1) * world_size + rank as u64) as i64;
        col1.push(val);
        col2.push((val * 7) % 1000); // Deterministic second column
    }
...
fn print_usage() {
    eprintln!("Usage:");
    eprintln!("  ./sorting_example m num_tuples_per_worker 0.0-1.0");
    eprintln!("  ./sorting_example f csv_file");
}
...
fn main() -> CylonResult<()> {
    ...
    // Perform distributed sort on column 0
    let sort_start_time = Instant::now();
    let sorted_table = distributed_sort(&input_table, 0, true)?;
    ...
    if sorted_table.rows() <= 1000 {
        if rank == 0 {
            println!("\nSorted table (first few rows):");
        }
        ctx.barrier()?;

        // Print first 10 rows from this rank
        if let Some(batch) = sorted_table.batch(0) {
            ...
            let rows_to_print = std::cmp::min(10, col1.len());
            for i in 0..rows_to_print {
                println!("Rank {}: {} | {}", rank, col1.value(i), col2.value(i));
            }
        }
    }
    ...
    if rank == 0 {
        println!("\nSorting example completed successfully!");
    }
