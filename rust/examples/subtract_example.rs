...
//! Distributed Subtract Example
//!
//! This example demonstrates distributed subtraction operations using MPI.
//! It reads two tables and performs a distributed subtract operation.
//!
//! Corresponds to cpp/src/examples/subtract_example.cpp
//!
//! Usage:
//!   mpirun -n 4 cargo run --example subtract_example --features mpi -- m 1000 0.5
//!   mpirun -n 4 cargo run --example subtract_example --features mpi -- f file1 file2
...
use cylon::ops::distributed_set_ops::distributed_subtract;
...
fn create_two_in_memory_tables(
    ...
    for i in 0..(dup_count / 2) {
        let val = ((i % (unique_count / 2).max(1)) * world_size + rank as u64) as i64;
        right_col1.push(val);
        right_col2.push(val * 10);
    }
...
fn print_usage() {
    eprintln!("Usage:");
    eprintln!("  ./subtract_example m num_tuples_per_worker 0.0-1.0");
    eprintln!("  ./subtract_example f csv_file1 csv_file2");
}
...
fn main() -> CylonResult<()> {
    ...
    // Perform distributed subtract
    let subtract_start_time = Instant::now();
    let subtract_table = distributed_subtract(&first_table, &second_table)?;
    let subtract_end_time = Instant::now();

    println!(
        "Rank {}: First table had: {} and Second table had: {}, Subtract has: {} rows",
        rank,
        first_table.rows(),
        second_table.rows(),
        subtract_table.rows()
    ...
    if rank == 0 {
        println!(
            "Subtract done in {}ms",
            subtract_end_time
                .duration_since(subtract_start_time)
                .as_millis()
        );
    }
    ...
    if rank == 0 {
        ...
        println!("Subtract example completed successfully!");
    }
